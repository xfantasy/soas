...
public class BasicRepositoryResource3 {
    ...
    static ResultSet rs=null;//初始化关于连接oracle的变量
    ...
    */
    ...
    public List<IRepositoryObject> getSaveList() throws SQLException{ //保存的
        ...
        try{
            ...
            rs=stmt.executeQuery("select SAVENAME from SAVEDATA");//获取已储存的名称

        while (rs.next()){
            SaveName.add(rs.getString("SAVENAME"));
        }
                return getSaveListObject(SaveName);
    ...
    @Produces({"text/plain" })
    @Path("/resource")
    public Response load_data(@QueryParam("file") String filetmp) throws SQLException{
        try{
    ...
    public Status save_data(@FormParam("file") String filetmp,
                            @FormParam("content") String content) throws  SQLException{
        try{
    ...
    @DELETE
    @Path("/resource")
    public Status delete_data(@QueryParam("file") String filetmp) throws SQLException{
        try{
    ...
    private List<IRepositoryObject> getSaveListObject(List<String> names){//要返回RepositoryFileObject\
        List<IRepositoryObject> saveListObject =new ArrayList<IRepositoryObject>();
        for(String tmp:names){
            saveListObject.add(new RepositoryFileObject(tmp,"#"+tmp,"saiku",tmp));//增加
        }
