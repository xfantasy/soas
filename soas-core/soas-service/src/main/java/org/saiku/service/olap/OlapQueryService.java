...
public class OlapQueryService implements Serializable {
    ...
    private final HigoQueryService higoQueryService = new HigoQueryService();

    private QueryScheduler queryScheduler = null;

    public QueryScheduler getQueryScheduler() {
        if(queryScheduler==null){
            Properties propertes = ConstantUtil.getProperties();
            try{
                queryScheduler = QueryScheduler.getInstance("pas2", propertes);
            }catch(Exception e){
                 log.error("get queryScheduler error!",e);
            }
...
//			System.out.println("Execute: ID " + Thread.currentThread().getId() + " Name: " + Thread.currentThread().getName());
            ...
            //TODO 临时测试增加higo处理
            if (query.getCube().getName().equals("Higo_DemoCube")) {
                result = higoQueryService.higo_execute(query,getQueryScheduler());
                if (result == null) {
                    result = new CellDataSet(0, 0);
                }
            } else {
                ...
                result = OlapResultSetUtil.cellSet2Matrix(cellSet, formatter);
